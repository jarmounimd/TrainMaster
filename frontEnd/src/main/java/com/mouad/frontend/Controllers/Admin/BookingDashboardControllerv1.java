...
//                }
    ...
    private void processRecord(String message) {
        try {
            ...
            System.out.println("Processing new message: " + message); // Debug log
            ...
            System.out.println("Logger: " + loggerName + ", Message: " + messageContent); // Debug log
            ...
            if (loggerName.equals("BookingController") &&
                ...
                System.out.println("Skipping message due to context: " + contextData); // Debug log
            ...
            if (loggerName.equals("BookingController")) {
                ...
                if (messageContent.contains("user email")) {
                    ...
                    System.out.println("Found user email in message. Extracted: " + userEmail); // Debug log
                    if (!userEmail.isEmpty()) {
                        key = new MessageKey(loggerName, userEmail);
                        userBookingCounts.put(key, logCount);
                        System.out.println("Updated user booking count: " + userEmail + " -> " + logCount); // Debug log
                    }
                }
                ...
                if (messageContent.contains("trip:")) {
                    ...
                    System.out.println("Updated route count: " + route + " -> " + logCount); // Debug log
                }
                ...
                if (messageContent.contains("timing:")) {
                    ...
                    System.out.println("Updated duration count: " + duration + " -> " + logCount); // Debug log
                }
                ...
                if (messageContent.contains("arrive:")) {
                    ...
                    System.out.println("Updated destination count: " + destination + " -> " + logCount); // Debug log
                }
                ...
                if (messageContent.contains("depart:")) {
                    ...
                    System.out.println("Updated departure count: " + departure + " -> " + logCount); // Debug log
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private String extractUserEmail(String message) {
        System.out.println("Raw message: " + message); // Debug log
        
        int startIndex = message.indexOf("user email");
        if (startIndex == -1) {
            System.out.println("Could not find 'user email' in message"); // Debug log
            return "";
        }
        
        startIndex += "user email".length();
        // Skip any whitespace and colon
        while (startIndex < message.length() && (Character.isWhitespace(message.charAt(startIndex)) || message.charAt(startIndex) == ':')) {
            startIndex++;
        }
        
        if (startIndex >= message.length()) {
            System.out.println("Reached end of message after skipping whitespace"); // Debug log
            return "";
        }
        
        // Find the end of the email (next whitespace or end of string)
        int endIndex = message.indexOf(" ", startIndex);
        String email = message.substring(startIndex, endIndex != -1 ? endIndex : message.length()).trim();
